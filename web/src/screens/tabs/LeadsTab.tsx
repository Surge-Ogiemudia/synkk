...
import { getLeads, updateLeadStatus as persistLeadStatus } from '@/lib/leads';
import { acceptLead } from '@/lib/api';
...
export default function LeadsTab() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('refresh-leads-list', refreshLeads);
    return () => {
      window.removeEventListener('refresh-leads-list', refreshLeads);
    };
  }, []);

  const updateStatus = async (leadId: string, status: 'accepted' | 'ignored', items: any[]) => {
    persistLeadStatus(leadId, status);
    ...
    if (status === 'accepted') {
      try {
        await acceptLead(slug, leadId, items);
      } catch (err) {
  ...
  const renderLead = (lead: any) => {
    ...
    return (
      ...
      >
        <div
          className={`absolute top-0 left-0 w-1 h-full ${isAccepted ? 'bg-emerald-500' : lead.status === 'ignored' ? 'bg-slate-600' : 'bg-emerald-400 animate-pulse'}`}
        />
        ...
        <div className={`flex justify-between items-start ${isExpanded ? 'mb-3' : 'mb-0'}`}>
          <div>
            ...
            {!isExpanded && !isPending && (
              <span
                className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider mt-2 inline-block ${isAccepted ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}
              >
                {lead.status}
              </span>
            )}
            ...
            {isPending ? (
              <div className="mt-auto space-y-3">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={(e) => { e.stopPropagation(); updateStatus(lead.id, 'ignored', lead.medicines); }}
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); updateStatus(lead.id, 'accepted', lead.medicines); }}
  ...
  return (
    <div className="flex flex-col w-full space-y-8 pb-10 overflow-y-auto">
      <div className="space-y-4">{pendingLeads.map(renderLead)}</div>
