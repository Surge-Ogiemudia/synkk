...
import { fetchPendingOrders, updateOrderStatus } from '@/lib/api';
...
export default function OrdersTab({ slug }: { slug: string }) {
  ...
  const fetchOrders = async () => {
    try {
      const data = await fetchPendingOrders(slug);
      if (data.success) {
        setOrders((prev) => {
          const processedList = Array.from(globalProcessedOrders.values());
          const fetchedPending = data.orders.filter((o: any) => !globalProcessedOrders.has(o._id));
  ...
  const renderOrder = (order: any) => {
    ...
    return (
      ...
      >
        <div
          className={`absolute top-0 left-0 w-1 h-full ${order.status === 'Accepted' ? 'bg-emerald-500' : order.status === 'Declined' ? 'bg-rose-500' : 'bg-amber-500'}`}
        />
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              ...
              {!isExpanded && !isPending && (
                <span
                  className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase tracking-wider ${order.status === 'Accepted' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-rose-500/20 text-rose-400'}`}
                >
                  {order.status}
                </span>
              )}
              ...
              {order.items?.map((item: any, idx: number) => (
                <div key={idx} className="flex justify-between text-sm py-1 border-b border-slate-800/50 last:border-0">
                  <span className="text-slate-300">
                    <span className="text-emerald-500 font-medium mr-1">{item.qty}x</span> {item.name}
                  </span>
  ...
  return (
    <div className="flex flex-col w-full space-y-8 pb-10 overflow-y-auto">
      <div className="space-y-4">{pendingOrders.map(renderOrder)}</div>
