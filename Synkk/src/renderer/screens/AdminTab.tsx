...
import { Search, Shield, Lock, Unlock, RefreshCw, CheckCircle2, Building2 } from 'lucide-react';
...
export default function AdminTab() {
  ...
  const fetchPharmacies = async (search: string) => {
    ...
    try {
      ...
      const data = await ipcRenderer.invoke('get-admin-pharmacies', search);
      if (data && Array.isArray(data)) {
        setPharmacies(data);
        return;
      }
    } catch (err) {
      console.warn('[AdminTab] Using default session storefront data:', err);
    } finally {
      setLoading(false);
    }
    ...
    try {
      ...
      const activeSlug = storefront?.slug || 'medlife';
      const activeName = storefront?.name || 'MedLife Pharmacy';
      ...
      setPharmacies([
        {
          ...
          name: activeName,
          slug: activeSlug,
          email: storefront?.email || 'admin@pharmastackx.com',
          allowedModules: {
            psxWeb: true,
            pos: true,
            emr: true,
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-800">
        <div>
          ...
          <p className="text-slate-400 text-sm">
            Manage master module entitlements across all pharmacy accounts. Locking a module disables it in the pharmacy's terminal settings and blocks access.
          </p>
        </div>
